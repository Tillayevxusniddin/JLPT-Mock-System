...
"""
Group membership utilities. History is recorded when any member (student or teacher)
is removed or moved, via GroupMembershipHistory.
"""
...
@transaction.atomic
def record_membership_removal_and_delete(membership: GroupMembership, reason: str = "REMOVED"):
    """
    Record the removal in GroupMembershipHistory and delete the membership.
    Use for both STUDENT and TEACHER to keep a consistent audit trail.

    Args:
        membership: The GroupMembership instance to remove.
...
def remove_student_from_group(user_id, actor, reason="REMOVED", group_id=None):
    ...
    Uses record_membership_removal_and_delete for consistent history.
    ...
    if actor.role != User.Role.CENTERADMIN:
    ...
    record_membership_removal_and_delete(membership, reason=reason)
